classdef Neuron < handle
    ...
    %% Algebraic Expressions
            ...
            J_Na_tot_sa =  J_Naleak_sa + J_Napump_sa + J_NaT_sa;%J_NaP_sa +
            ...
            J_Na_tot_d  =  J_Naleak_d + J_Napump_d+ J_NMDA_Na_d ;%J_NaP_d +
...
%             Rad             = p.R_init.*exp(-(((K_e)-3.5)./p.a_constr)).*((1+p.b_maxdil.*exp(-(((K_e)-10)./p.c_dilres).^2))./(1+p.b_maxdil.*exp(-(6.5./p.c_dilres).^2)));
%             CBF             = p.CBF_init * (Rad.^4 / p.R_init^4);
      ...
      %% Conservation equations                                                         
       ...
       % Current input to neuron
        function current = input_current(self, t) 
            p = self.params;
            current = p.Istrength * rectpuls(t - (p.t_0 + p.lengthpulse/2), p.lengthpulse)+ p.Istrength * rectpuls(t - (p.t_0 + 50), 20)+ p.Istrength * rectpuls(t - (p.t_0 + 90), 20)...
                + p.Istrength * rectpuls(t - (p.t_0 + 130), 20) + p.Istrength * rectpuls(t - (p.t_0 + 170), 20);   
...
function params = parse_inputs(varargin)
    ...
    parser.addParameter('SC_coup', 20);        % [-] Coupling scaling factor, values can range between 1.06 to 14.95 according to estimation from experimental data of Ventura and Harris, Maximum dilation at 9.5
    ...
    parser.addParameter('a_constr', 50);
    parser.addParameter('b_maxdil', 2.5);
    parser.addParameter('c_dilres', 3);
...
%     parser.addParameter('Imax', 0.013*4);  
    ...
    parser.addParameter('gNaleak_sa', 4.1893e-5);   %without NaP channel
    parser.addParameter('gKleak_sa', 1.4555e-4);
    parser.addParameter('gleak_sa', 10*4.1893e-5);
    parser.addParameter('gNaleak_d', 4.1808e-5);   
    parser.addParameter('gKleak_d', 1.4553e-4); 
    parser.addParameter('gleak_d', 10*4.1808e-5);  
    parser.addParameter('Imax', 0.013*4); 
...
%     parser.addParameter('gNaleak_sa', 1.1104e-4);   %without NaP channel
%     parser.addParameter('gKleak_sa', 3.8756e-4);
%     parser.addParameter('gleak_sa', 10*1.1104e-4);
%     parser.addParameter('gNaleak_d', 1.1095e-4);   
%     parser.addParameter('gKleak_d', 3.8754e-4); 
%     parser.addParameter('gleak_d', 10*1.1104e-4);  
%     parser.addParameter('Imax', 0.013*10);
    
    
%     parser.addParameter('gNaleak_sa', 8.288e-5);   %without NaP channel
%     parser.addParameter('gKleak_sa', 3.0983e-4);
%     parser.addParameter('gleak_sa', 10*8.288e-5);
%     parser.addParameter('gNaleak_d', 8.8743e-5);   
%     parser.addParameter('gKleak_d', 3.0981e-4); 
%     parser.addParameter('gleak_d', 10*8.8743e-5);  
%     parser.addParameter('Imax', 0.013*8);

%     parser.addParameter('gNaleak_sa', 7.7718e-5);   %without NaP channel
%     parser.addParameter('gKleak_sa', 2.7094e-4);
%     parser.addParameter('gleak_sa', 10*7.7718e-5);
%     parser.addParameter('gNaleak_d', 7.7632e-5);   
%     parser.addParameter('gKleak_d', 2.7094e-4); 
%     parser.addParameter('gleak_d', 10*7.7632e-5);  
%     parser.addParameter('Imax', 0.013*7);

%     
%     parser.addParameter('gNaleak_sa', 6.2841e-5);   %without NaP channel
%     parser.addParameter('gKleak_sa', 2.1887e-4);
%     parser.addParameter('gleak_sa', 10*6.2841e-5);
%     parser.addParameter('gNaleak_d', 6.2756e-5);   
%     parser.addParameter('gKleak_d', 2.1885e-4); 
%     parser.addParameter('gleak_d', 10*6.2756e-5);  
%     parser.addParameter('Imax', 0.013*6);

%     parser.addParameter('gNaleak_sa', 5.2367e-5);   %without NaP channel
%     parser.addParameter('gKleak_sa', 1.8221e-4);
%     parser.addParameter('gleak_sa', 10*5.2367e-5);
%     parser.addParameter('gNaleak_d', 5.2282e-5);   
%     parser.addParameter('gKleak_d', 1.8219e-4); 
%     parser.addParameter('gleak_d', 10*5.2282e-5);  
%     parser.addParameter('Imax', 0.013*5);
